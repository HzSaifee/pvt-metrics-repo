...
WHERE
    ...
    AND CAST(wd_event_date as TIMESTAMP) >= CAST(date_add('day', -{{ lookback_days }}, current_date) AS TIMESTAMP)
    AND wd_event_date like '2%'
    AND job_definition in ({{ job_definitions }})
