-- =============================================================================
-- TAM-SAM Penetration Datasource - PER TOOL VIEW
-- =============================================================================
-- Purpose: Combined datasource for penetration percentage calculations
--          With tool-type breakdown for per-tool analysis
--
-- Row Types:
--   - ACTIVITY: Tool activity rows (for penetration numerator) - per tool
...
--   Activity rows include tool_type column for filtering/grouping
--   Each customer-tool combination appears at ONE date only
--
-- Penetration Calculation (in Tableau):
--   Filter by tool_type first, then:
...
-- =============================================================================
-- PART A: ACTIVITY DATA PER TOOL (Numerator for Penetration)
-- Adapted from activity_datasource_per_tool.sql
-- =============================================================================
...
-- Per-tool: First created per customer-tool
...
-- Per-tool: First migrated per customer-tool
...
active_customers_base AS (
    ...
    WHERE sct.tenant_type = 'Production'
      ...
      AND (
          -- Include customers with NO deployments
          acd.customer_sf_account_id IS NULL
          OR (
              -- Include customers with valid deployments (exclude certain phases/products)
...
ORDER BY row_type, user_type, tool_type NULLS LAST, sf_account_id
